...
export const ${method.name}Thunk = createAsyncThunk('${model.name}/${method.name}', async function(arg: <@methodParameterType method=method emptyParameterType="undefined"/>, thunkAPI): <#if method.returnTypeWrapper == 'Observable'>Promise
<DownloadStreamDTO
<Uint8Array>><#else>${method.returnTypeWrapper}<${method.returnType.name}></#if> {
    <#if method.parameters?has_content>
        const { ${method.parameters?map(param -> param.name)?join(", ")} } = arg;
    </#if>

    const headers =
    prepareHeaders(thunkAPI.getState()<#if method.bodyParameter?has_content>, { contentType: <#if method.formData>'multipart'<#else>'json'</#if> }<#elseif method.formData>, { contentType: 'formData' }</#if>
    );
    ...
    <string | number | boolean>> = {};
        ...
        <#if method.returnType.name != "void">return </#if>await
        fetch(url(<@url method=method state="thunkAPI.getState()"></@url><#if method.queryParams?has_content && (method.httpMethod == "GET" || method.httpMethod == "DELETE")>, params</#if>
        ), {
        ...
        map<#switch method.returnType.name><#case 'void'>VoidResponse<#break><#case 'string'>StringResponse<#break><#case 'DownloadInfo'>StreamingResponse<#break><#default>JsonResponse<${method.returnType.name}></#switch>
        (res));
        }, ThunkOptions);
        </#list>
