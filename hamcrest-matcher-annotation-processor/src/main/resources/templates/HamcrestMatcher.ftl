...
<#-- @ftlvariable name="entityType" type="java.lang.String" -->
...
<#-- @ftlvariable name="fields" type="java.util.List<api.maven.plugin.processor.hamcrest.matcher.data.ClassField>" -->
package ${packageName};
...
public class ${entityName}Matcher${generics} extends TypeSafeMatcher<${entityType}${generics}> {
...
<#list fields as field>
    ...
    @Override
    protected boolean matchesSafely(${entityType}${generics} entity) {
        return ${fields?map(field -> "this." + field.name + ".matches(entity." + field.getter + "())")?join("\n\t\t\t&& ")};
    }

    @Override
    public void describeTo(Description description) {
        description.appendText(${"\"" + entityType + "(\""})
            ${fields?map(field -> ".appendText(\"" + field.name + "=\").appendDescriptionOf(this." + field.name + ")")?join(".appendText(\", \")\n\t\t\t")}
            .appendText(${"\")\""});
    }
<#list fields as field>
