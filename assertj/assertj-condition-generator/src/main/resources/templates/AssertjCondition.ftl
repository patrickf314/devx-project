...
<#list condition.fields as field>
    ...
    private static Description propertyDescription(String property, Description description) {
        return new TextDescription(property + "=" + (description == null ? "<any>" : description.value()));
    }

    public static ${condition.generics?has_content?then(condition.generics + ' ', '')}${condition.className}Condition${condition.generics} ${conditionFactoryFunctionName}() {
