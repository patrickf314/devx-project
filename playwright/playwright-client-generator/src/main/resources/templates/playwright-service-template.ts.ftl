...
async ${method.name}(<#if testContextIdentifier?has_content>context: ${testContextIdentifier} | undefined, </#if>arg: <@methodParameterType method=method emptyParameterType="undefined"/>): <#if method.returnTypeWrapper == 'Observable'>Promise
<Buffer><#else>${method.returnTypeWrapper}<${method.returnType.name}></#if> {
    ...
    <string | number | boolean>> = {};
        ...
        this.request.fetch(url(<@url method=method></@url><#if method.queryParams?has_content && (method.httpMethod == "GET" || method.httpMethod == "DELETE")>, params</#if>
        ), {
