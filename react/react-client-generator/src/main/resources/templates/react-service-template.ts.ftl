...
<Uint8Array>><#else>${method.returnTypeWrapper}<${method.returnType.name}></#if> {
    ...
    <string | number | boolean>> = {};
        ...
        map<#if method.returnTypeWrapper == 'Observable'>StreamingResponse<#else><#switch method.returnType.name><#case 'void'>VoidResponse<#break><#case 'string'>StringResponse<#break><#case 'DownloadInfo'>StreamingResponse<#break><#default>JsonResponse<${method.returnType.name}></#switch></#if>
        (res));
        }, ThunkOptions);
        </#list>
